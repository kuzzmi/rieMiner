...
void xptClient_sendWorkerLogin(xptClient_t* xptClient)
{
	...
	xptPacketbuffer_writeU32(xptClient->sendBuffer, &sendError, xptClient->payload);							// payloadNum (removed in version 6)
...
void xptClient_sendShare(xptClient_t* xptClient, xptShareToSubmit_t* xptShareToSubmit)
{
	...
	uint256 testmerkle;
	std::vector<uint8> vectorMerkle;
	for (int u = 0; u < 32; u++) {
		vectorMerkle.push_back(xptShareToSubmit->merkleRoot[u]);
	}
		
	testmerkle = uint256(vectorMerkle);
	int i1 = 1+1;
	...
	{
		xptPacketbuffer_writeData(xptClient->sendBuffer, xptShareToSubmit->nOffset, 32, &sendError);
	}
