...
void xptMiner_submitShare(minerRiecoinBlock_t* block)
{
	...
	xptShare->version = block->version;
	xptShare->nTime = block->timestamp;
	
	memcpy(xptShare->nOffset, &block->nOffset, 32);
...
void xptMiner_getWorkFromXPTConnection(xptClient_t* xptClient)
{
	...
	// set payloads
	 ...
	 {		 
		for( int iPayload = 0; iPayload < xptClient->payload; iPayload++ )
		{			
			memcpy(&workDataSource.rieMerkle[iPayload].blockHash, xptClient->blockWorkInfo.merklePayload[iPayload].blockHash, 32);
			memcpy(&workDataSource.rieMerkle[iPayload].merkleRoot, xptClient->blockWorkInfo.merklePayload[iPayload].merkleRoot, 32);
		}
	 }
	...
	std::cout << "New Block: " << monitorCurrentBlockHeight << "\n";
}
...
void xptMiner_xptQueryWorkLoop()
{
	...
	xptClient->payload = minerSettings.payload;
	...
	while( true )
	{
		...
		if( currentTick >= timerPrintDetails )
		{
			...
			if( xptClient_isDisconnected(xptClient, NULL) == false )
			{
				...
				else if( workDataSource.algorithm == ALGORITHM_METISCOIN || workDataSource.algorithm == ALGORITHM_MAXCOIN || workDataSource.algorithm == ALGORITHM_RIECOIN )
				{
		...
		if( xptClient_isDisconnected(xptClient, NULL) == false )
		{
			...
			{
				// is known algorithm?
				if( xptClient->clientState == XPT_CLIENT_STATE_LOGGED_IN && (xptClient->algorithm != ALGORITHM_PROTOSHARES && xptClient->algorithm != ALGORITHM_RIECOIN && xptClient->algorithm != ALGORITHM_SCRYPT && xptClient->algorithm != ALGORITHM_METISCOIN && xptClient->algorithm != ALGORITHM_MAXCOIN) )
				{
...
{
	commandlineInput.host = "candypool.net";
	srand(GetTickCount());
	...
	minerSettings.payload = commandlineInput.numThreads;
